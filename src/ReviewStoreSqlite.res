...
let rowToReview = (json: JSON.t): ReviewStoreTypes.review => {
  let r = S.convertOrThrow(json, ReviewStoreTypes.reviewSchema)
  {
    ...r,
...
let getForMedia = (~mediaKey, ~excludeDid: option<Handle.t>=?) => {
  ...
  rows->Array.map(rowToReview)->Promise.resolve
}
